...
Enemy movement uses direct server-authoritative chasing toward the nearest alive player. Solid saved-map tiles and layer-3 table objects block movement and line-of-sight checks; when an enemy's direct segment to its target is blocked, the enemy idles instead of computing a route around the blocker. Caltrops are not route blockers; they only apply their server-side slow when enemies physically cross them. Map tile and layer-3 table topology changes invalidate collision caches used by enemy movement checks.
