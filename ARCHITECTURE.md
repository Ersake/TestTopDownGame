...
- Wood shield and bone shield are craftable server-owned hotbar items. Wood shield costs 10 wood and has 10 shield HP; bone shield costs 10 bones and has 20 shield HP. Enemy kills add bone stacks directly to the killer's hotbar. Left-click with either shield sends normal attack intent, and the server applies a short-cooldown 1-damage circular bash in front of the player from the cursor aim direction. Right-click with an equipped shield sends held block start/stop intent. While server-validated blocking is active, incoming player damage is applied to the active shield slot before health; damage beyond remaining shield HP spills over to player health through the normal hurt/death path. Shield HP is stored per hotbar slot, moves with slot swaps, regenerates by 1 HP every 3 seconds in any slot, and breaking a shield starts a 5-second server-owned block cooldown rendered on shield hotbar slots.
